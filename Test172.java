...
○ 문자 스트림(character-stream) 입출력
   ...
   - Reader 클래스와 Writer클래스, 그리고 이들의 하위 클래스에 의해
...
※ 스트림의 특징
  ...
  - 스트림은 FIFO 구조이다.
	...
	스트림의 데이터는 순차적으로 흘러가며 순차적 접근 밖에 허용되지 않는다.
  ...
  - 스트림은 지연될 수 있다.
    스트림에 넣어진 데이터는 처리되기 전에 스트림에 사용하는 스레드는
	지연(blocking) 상태에 빠진다.
	네트워크 상에서는 데이터가 모두 전송되기까지 스레드는 지연(blocking) 
	상태가 된다.
*/
