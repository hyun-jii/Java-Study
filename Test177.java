...
○ 객체 직렬화(Object Serialization)는
   ...
   멤버 변수의 값을 보존한다거나
   다른 네트워크에 있는 호스트에게 값을 보낼 경우 사용하게 된다.
...
○ 객체 직렬화의 장점
   ...
   이 객체 직렬화를 사용하면 편하게 객체 상태를 저장할 수 있다.
    ...
    ※ Bean(빈)
	   C/S(Client와 Server) 구조적 모델에서 서버측 구조에 해당하며,
...
○ Object Stream
   ...
   스트립으로부터 직렬화된 객체를 읽을 수 있으며

   이렇게 읽는 객체는 배열, 문자열 또는 각 객체 등
...
public class Test177
{
	public static void main(String[] args) throws Exception
	{
		...
		//--==>> C:\JavaStudy\JavaStudy\JavaStudy

		//파일 객체 생성
		File f0  = new File(appDir, "\\data\\test.ser");
		//appDir -> C:\JavaStudy\JavaStudy\JavaStudy
		//appDir 위치를 중심으로 "\\data\\test.ser"를 구성하겠다는 의미,
		//결과적으로 C:\JavaStudy\JavaStudy\JavaStudy\data\test.ser 구성

		//확인 1
		//System.out.println(f0.getParentFile().exists());
		//--==>> false

		//확인 2 -> C:\JavaStudy\JavaStudy\JavaStudy 경로에 test 디렉터리 생성 후 확인
		//System.out.println(f0.getParentFile().exists());
		//--==>> false

		//확인 3 -> C:\JavaStudy\JavaStudy\JavaStudy 경로에 data 디렉터리 생성 후 확인
		//System.out.println(f0.getParentFile().exists());
		//--==>> true

		

		if (!f0.getParentFile().exists())  //디렉터리 구성(C:\JavaStudy\JavaStudy\JavaStudy\data)이 존재하지 않는 상황이라면...
		{
		...
		//-- 파일 전용 출력 스트림(물줄기)에 f0 이라는 파일 객체를 띄우겠다...
		...
		if (f0.exists())        //-- f0 파일 객체가 존재한다면...
		{
			...
			// ois로 부터 읽어들인 객체(Object)를 캐스팅(Hashtable) 하여
			// h2 라는 Hashtable 자료구조에 담아내기
			Hashtable h2 = (Hashtable)ois.readObject();


			ois.close();         //-- ois 반납 -> ObjectInputStream 리소스 반납
			fis.close();		 //-- fis 반납 -> FileInputStream 리소스 반납
			...
			while (e.hasMoreElements())
			{
				...
				value = (String)h2.get(key);      //key를 활용하여 value를 얻는 과정
